...
void neighbors_init() {
   ...
   for (i=0;i<MAXNUMNEIGHBORS;i++){
      ...
      neighbors_vars.neighbors[i].mask=0xffff;
      for(uint8_t j=0; j<15; j++){
          neighbors_vars.neighbors[i].usable[j]=1;
      }  
      /***************/
   }
...
//address type should be ADDR_64B because it's MAC now
   ...
   for (i=0;i<MAXNUMNEIGHBORS;i++) {
      if (isThisRowMatching(neighbor,i)) {
         return neighbors_vars.neighbors[i].mask;
      }
   }
   return 0xFFFF;
}
...
void neighbors_setMask(open_addr_t* neighbor, uint16_t newMask){
   ...
   for (i=0;i<MAXNUMNEIGHBORS;i++) {
      if (isThisRowMatching(neighbor,i)) {
        neighbors_vars.neighbors[i].mask = newMask;
      }
   }
   return;
}
